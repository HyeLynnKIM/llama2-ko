...
def inference(args):
    ...
    prompt = f"""당신은 많은 지식을 가진 유능한 어시스턴트 입니다. 주어지는 질문에 대한 답변을 해주세요. 단, 답변은 세 줄까지만 출력하세요.
        ### 질문: {ques}\n\n### 답변:
        """
    pipe = pipeline(task="text-generation", model=model, tokenizer=tokenizer, max_length=512, eos_token_id=2, do_sample=True, num_return_sequences=5, top_k=5, temperature=0.2)
    result = pipe(prompt)
